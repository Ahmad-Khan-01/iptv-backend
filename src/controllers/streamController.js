...
export const streamController = {
  ...
  getEpisodeOfStreamByStreamId: async (req, res) => {
    try {
      const data = await streamService.getEpisodeOfStreamByStreamId(
        req.params.id
      );
      if (!data) {
  ...
  getSeasonOfEpisodeOfStreamByStreamId: async (req, res) => {
    try {
      const data = await streamService.getSeasonOfEpisodeOfStreamByStreamId(
        req.params.id
      );
      if (!data) {
        return httpResponse.NOT_FOUND("Data not found");
  ...
  getSeriesOfSeasonOfEpisodeofStreamByStreamId: async (req, res) => {
    try {
      const data =
        await streamService.getSeriesOfSeasonOfEpisodeofStreamByStreamId(
          req.params.id
        );
      if (!data) {
        return httpResponse.NOT_FOUND("Data not found");
  ...
  getGenreOfSeriesOfSeasonOfEpisodeOfStreamByStreamId: async (req, res) => {
    try {
      const data =
        await streamService.getGenreOfSeriesOfSeasonOfEpisodeOfStreamByStreamId(
          req.params.id
        );
      if (!data) {
        return httpResponse.NOT_FOUND("Data not found");
